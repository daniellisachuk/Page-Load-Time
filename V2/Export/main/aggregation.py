...
#TODO
# - calc loss from pcap ------------------------------------------------ V
# - calc inter-packet time ---------------------------------------------
# - calc initial RTT (rtt of 3-way handshake) -------------------------- V
# - calc avg RTT ------------------------------------------------------- V
# - calc TTFB ---------------------------------------------------------- V
...
class Aggregation:
    ...
    def __init__(self, core_domain, disconnect_threshold, v_verbose=False):
    ...
    def add_actual_syn(self, local_port: int, ip: str, remote_port: int, timestamp: float):
        ...
        if self.first_syn_time is None and self.core_dns_q_pkt:
            self.first_syn_time = timestamp

        # check if still expecting syns
        flag1 = False
        if self.dns_time:
            # if past syn time
